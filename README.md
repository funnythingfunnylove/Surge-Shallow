...
- 内置软件更新器：启动时或手动检查 GitHub 最新正式 Release，发现新版本后展示完整更新日志；点击“立即更新并重启”会下载 macOS ZIP，并依次校验 GitHub SHA-256、Bundle ID、版本号和代码签名完整性，再带回滚保护替换当前应用。安装目录需要写入权限时会弹出 macOS 管理员授权。
...
软件自身的更新可从“Surge Shallow → 检查更新…”，菜单栏面板或“设置 → 软件更新”手动触发；默认会在启动时检查，并在持续运行期间每 6 小时检查一次 GitHub Release。发现新版本后先阅读弹出的更新日志，再点击“立即更新并重启”。从只读磁盘映像运行时，请先把 App 移到本机磁盘。
