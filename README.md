...
1. 打开 Profiles 页面，通过“添加通用选项”配置两端共同使用的 General 项；高级公共段继续在下方编辑器中维护。
2. 打开 Proxy 页面，添加公共 Proxy 和 Proxy Group，填写名称、协议或组类型及其参数。
3. 回到 Profiles，在 macOS 与 iOS/iPadOS 卡片中通过“添加差异项”选择覆盖项或平台专属项；没有差异时保持空列表即可完整继承公共配置。
4. 确认公共 `.dconf` 与两个输出 `.conf` 的文件名不会和手工维护的文件重名。
5. 在规则源页面按期望优先级添加 URL，选择目标策略与平台。
6. 点击“立即更新并合并”。
7. 在总览中确认 Surge CLI 校验结果、规则数和去重数。
8. 在 Surge 的配置列表中选择生成文件；其他设备等待 iCloud 完成同步即可。
...
测试覆盖规则格式、策略处理、逻辑规则、去重、平台过滤、Detached Profile 共享、平台差异叠加、旧 schema 迁移、真实 `surge-cli` 检查、FINAL 门禁、缓存回退、发布门禁和非 Relay 文件防覆盖。
